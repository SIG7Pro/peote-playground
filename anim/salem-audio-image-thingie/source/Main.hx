...
// Taken from https://github.com/openfl/lime-samples/blob/master/features/graphics/SimpleImage/Source/Main.hx, testing reasons only.
...
class Main extends Application {
	...
	var soundWaveFiles:Array<String> = [
		...
		'assets/05.wav',
		'assets/06.wav',
		'assets/music/Guitar.wav',
		//'assets/music/Unrelated.wav',
	];
	...
	function spawnVisualizer()
	{
		...
		var soundBar = new SoundBar(wavTextureData[6], window.width>>1, window.height>>1, window.width, window.height, Color.WHITE, Color.WHITE, 0.5, peoteView.time);
		soundBar.play();
		//trace(' Progress overall: ' + Std.int(loaded / size * 100) + "%" , ' ($loaded / $size)');
		image = Assets.getImage("assets/images/Mirror.png");
		//add(image);
		trace("awyeah");/*
		// to test MULTIPLE:
		window.onMouseDown.add((x, y, button) -> { 
			var soundBar = new SoundBar(wavTextureData[rnd(1,6)], 60, 60, 800, 300, Color.BLUE, Color.WHITE, 0.5, peoteView.time);
			soundBar.play();
			//(wavTextureData[rnd(1,6)], Std.int(x), Std.int(y) ,300, 300, Color.WHITE, Color.WHITE, 0.5, peoteView.time);
			//new
			//(data:{buffer:AudioBuffer, start:Float, length:Float}, x:Int, y:Int, w:Int, h:Int, colorQuite:Color, colorLoud:Color, gain:Float, timeStart:Float = 0.0 )
			trace("Sound.");
		});
	*/
